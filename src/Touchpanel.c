...
static void TestTouchPanelOneWire(void);
void Test_Touchpanel(void)
{

	if (HasOneWire()) {
		TestTouchPanelOneWire();
		return;
	}
...
static void TestTouchPanelOneWire(void)
{
    Uart_Printf("\nOne-Wire touch screen test\n");
	Uart_Printf("\nType ESC key to exit!!!\n");
    while( Uart_GetKey() != ESC_KEY ) {
    	int r;
    	unsigned Pressed, x, y;
    	r = GetOneWirePoint(&Pressed, &x, &y);
    	if (r && Pressed) {
